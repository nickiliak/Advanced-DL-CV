...
class Diffusion:
    ...
    def get_betas(self, schedule='linear'):
        if schedule == 'linear':
            return torch.linspace(self.beta_start, self.beta_end, steps=self.T)
        else:
            points = torch.linspace(0, 1, steps=self.T)
            points = points * math.pi
            cos_points = torch.cos(points)

            min_cos = torch.cos(math.pi)
            max_cos = torch.cos(0)
            
            rescaled_betas = (cos_points - min_cos) / (max_cos - min_cos) * (self.beta_end - self.beta_start) + self.beta_start
    ...
    def q_sample(self, x, t):
        ...
        device = x.device
        sqrt_alpha_bar = torch.sqrt(self.alphas_bar[t]).to(device)
        sqrt_alpha_bar = sqrt_alpha_bar[:, None, None, None]

        sqrt_one_minus_alpha_bar = torch.sqrt(1 - self.alphas_bar[t]).to(device)
